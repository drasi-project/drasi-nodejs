...
[`package.json`](../package.json) exposes two explicit publish helpers:

| Script                   | What it does                                                        |
| ------------------------ | ------------------------------------------------------------------- |
| `npm run pack:verify`    | `npm pack --dry-run` — inspect the main package's tarball contents. |
| `npm run publish:public` | `npm publish --access public` — publish to public npm.              |

You usually do **not** need a local `.npmrc` for public publishing — with no
`.npmrc` present, npm defaults to the public registry, which is what CI uses. If
you want to pin the registry locally, copy
[`.npmrc.publish-public.example`](../.npmrc.publish-public.example) to a real
`.npmrc` (it is gitignored so it is never committed).

The normal OSS release is fully automated (see _The release pipeline_ above):
bump the version, update the changelog, and push a `vX.Y.Z` tag; the `publish`
job runs `npm publish --access public` with provenance. `publish:public` is a
manual fallback for the rare case you need to publish outside CI.
