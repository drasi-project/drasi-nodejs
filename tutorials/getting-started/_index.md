...
| **[Step 2: Run the Demo](#run)** | One command starts PostgreSQL and the console app | 3 min |
| **[Step 3: Watch Changes](#changes)** | Insert, update, and delete messages; watch `all-messages` react | 4 min |
| **[Step 4: Filter](#filter)** | See how a `WHERE` clause turns a change feed into a focused signal | 3 min |
| **[Step 5: Aggregate](#aggregate)** | Count messages live as data changes | 3 min |
| **[Step 6: Detect Inactivity](#time)** | Detect the *absence* of change over time | 4 min |
| **[Step 7: Join Across Sources](#join)** | Join messages with live location data from a second source | 5 min |
| **[How It Works](#how)** | Understand the sources, the five queries, and the JavaScript reaction | 6 min |
| **[Putting It All Together](#together)** | Read the whole app — every source, query, and the reaction — in one file | 3 min |
...
1. Open the [`drasi-nodejs`](https://github.com/drasi-project/drasi-nodejs) repository in VS Code and
   run **Reopen in Container** (or create a **Codespace** from the repo's **Code** menu).
2. When prompted for a configuration, choose **Drasi Node.js — Getting Started Tutorial**.
3. Wait for the container to finish. Its setup script installs Node dependencies for the tutorial.
...
query firing because the four seed senders have gone quiet (more on that in [Step 6](#time)). Leave
...
## Step 3 of 7: Watch Changes {#changes}
...
## Step 5 of 7: Aggregate {#aggregate}
...
## Step 6 of 7: Detect Inactivity {#time}
...
## Step 7 of 7: Join Across Sources {#join}
